...
class TeamList extends Component {
  ...
  renderUsers = (user, key) => {
    ...
    return (
      <div key={key} className={`user-container`} onClick={() => this.handleUserRemove(user)}>
    ...
    return (
      <Grid container spacing={0} className="teams-container">
        {teams.map((team, index) => (
          <Item key={index} team={team} handleSelect={this.handleSelect} users={team.users} renderUsers={this.renderUsers} />
        ))}
      </Grid>
    );
  }
...
const mS = ({ teamReducer: { teams }, userReducer: { users } }) => ({ teams, users });
...
export default withStyles(styles)(connect(mS,mD)(TeamList));